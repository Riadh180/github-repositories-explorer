...
import {
  ...
  useGetRepositoriesForUserLazyQuery,
  User,
...
const App: React.FC = () => {
  ...
  const [getUsers, { data: userData, loading: userLoading, error: userError }] =
    useGetUsersLazyQuery();
  const [
    getRepos,
    { data: repoData, loading: repoLoading, error: repoError, fetchMore },
  ] = useGetRepositoriesForUserLazyQuery();
  ...
  return (
    <Container maxWidth="sm">
      ...
      {userData?.search.edges && (
        <Typography style={{ margin: '20px 5px' }}>
          Showing users for {inputTerm}
        </Typography>
      )}
